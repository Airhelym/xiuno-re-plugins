...
function search_post_by_fulltext($keyword_decode_against, $start, $pagesize2, &$nextpage, &$page) {
	...
	static $call_count = 0;
	if($call_count++ > 5) return array();
	$arrlist = db_sql_find("SELECT * FROM bbs_post_search WHERE MATCH(message) AGAINST ('$keyword_decode_against' IN BOOLEAN MODE) ORDER BY pid DESC LIMIT $start, $pagesize2;");
...
if($keyword) {
	if($search_type == 'fulltext') {
		...
		if($range == 1) {
			...
			$total = search_user_count($keyword_decode);
			$pagination = pagination(url("search-$keyword-$range-{page}"), $total, $page, $pagesize);
			$userlist = search_user_by_username($keyword_decode, $page, $pagesize);
		}

	} elseif($search_type == 'like') {

		if($range == 1) {
			$like_keyword = search_fuzzy_enabled() ? search_fuzzy_split($keyword_decode) : '%'.$keyword_decode.'%';
			$arr = db_sql_find_one("SELECT COUNT(*) AS num FROM bbs_thread WHERE subject LIKE '$like_keyword'");
			$total = $arr ? intval($arr['num']) : 0;
			$pagination = pagination(url("search-$keyword-$range-{page}"), $total, $page, $pagesize);
			$threadlist = db_sql_find("SELECT * FROM bbs_thread WHERE subject LIKE '$like_keyword' ORDER BY tid DESC LIMIT ".(($page - 1) * $pagesize).", $pagesize;");
			$threadlist = arrlist_multisort($threadlist, 'tid', FALSE);
			foreach($threadlist as &$thread) {
			...
			$like_keyword = search_fuzzy_enabled() ? search_fuzzy_split($keyword_decode) : '%'.$keyword_decode.'%';
			$arr = db_sql_find_one("SELECT COUNT(*) AS num FROM bbs_post WHERE message LIKE '$like_keyword'");
			$total = $arr ? intval($arr['num']) : 0;
			$pagination = pagination(url("search-$keyword-$range-{page}"), $total, $page, $pagesize);
			$postlist = db_sql_find("SELECT * FROM bbs_post WHERE message LIKE '$like_keyword' ORDER BY pid DESC LIMIT ".(($page - 1) * $pagesize).", $pagesize;");
			$postlist = arrlist_multisort($postlist, 'pid', FALSE);
			foreach($postlist as &$post) {
				post_format($post);
				$post['message_fmt'] = search_message_format($post['message_fmt']);
...
if($ajax) {
	if($threadlist) {
		...
		foreach($userlist as &$user) $user = user_safe_info($user);
		message(0, $userlist);
	}
} else {
	include _include(APP_PATH.'plugin/xn_search_re/htm/search.htm');
}
